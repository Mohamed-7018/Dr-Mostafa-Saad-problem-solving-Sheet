...
void solve(){
  ...
  int y2 = stoi(ss.substr(0, 4));
  
  int m1 = stoi(s.substr(5, 7));
  int m2 = stoi(ss.substr(5, 7));
  
  int d1 = stoi(s.substr(8, 11));
  int d2 = stoi(ss.substr(8, 11));
  
  int yy=y1; int mm = m1; int dd = d1;
  
  ll ans =0;
  while(true){
    if (yy == y2 && mm==m2 && dd==d2) break;
    
    if(isleap(yy)) months[2] = 29 ;
    else months [2] = 28;
    
    if(dd == months[mm] && mm != 12){
      dd = 1;
      mm++;
    }
    else if(dd== months[mm] && mm == 12){
      dd = 1;
      mm = 1;
      yy++;
    }
